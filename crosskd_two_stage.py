...
class CrossKDTwoStageDetector(TwoStageDetector):
    ...
    def __init__(self,
        ...
        self.reused_teacher_head_idx = kd_cfg['reused_teacher_head_idx'] # Lấy đặc trưng trung gian
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        # Căn chỉnh các tensor về cùng một kích thước nếu cần thiết
        aligned_stu_0 = F.interpolate(stu_x[0], size=stu_x[self.reused_teacher_head_idx].shape[2:], mode='bilinear', align_corners=False)
        aligned_stu_1 = F.interpolate(stu_x[1], size=stu_x[self.reused_teacher_head_idx].shape[2:], mode='bilinear', align_corners=False)
        aligned_stu_2 = F.interpolate(stu_x[2], size=stu_x[self.reused_teacher_head_idx].shape[2:], mode='bilinear', align_corners=False)
        aligned_stu_3 = F.interpolate(stu_x[3], size=stu_x[self.reused_teacher_head_idx].shape[2:], mode='bilinear', align_corners=False)

        # Cộng các tensor lại
        T = aligned_stu_0 + aligned_stu_1 + aligned_stu_2 + aligned_stu_3 + stu_x[self.reused_teacher_head_idx]
